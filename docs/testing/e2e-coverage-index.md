...
|---|---|---|---|---|
| M365 Customer Graph Read Phase 2 | `60bfbb5f4` (branch `feat/m365-customer-graph-read-consent`) | 2026-07-14 | [real-tenant acceptance checklist](../runbooks/m365-customer-graph-read-real-tenant.md) | **NEEDS-CREDS / NOT RUN** — disposable tenant, two Breeze orgs, eligible/ineligible admins, consent-copy capture, replay/expiry/ownership/drift/re-consent/revocation/outage/disconnect, cleanup, and secret non-observation evidence. Automated coverage exists on the feature branch; this row is the live-tenant gate. |
...
- **NEEDS-CREDS** — [M365 Customer Graph Read real-tenant acceptance](../runbooks/m365-customer-graph-read-real-tenant.md); Google Workspace + legacy M365 real-tenant flows (connect, offboard/wipe, drift dashboard, helpdesk tools, OData escaping on live data); live Win/macOS agent items (macOS `.pkg` sig verify, quarantine re-enroll, remote-desktop self-heal/revocation); SSRF live egress trigger.
