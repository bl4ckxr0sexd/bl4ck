...
const VAULT_REF = /^akv:\/\/([a-z0-9](?:[a-z0-9.-]*[a-z0-9])?)\/m365-comms-client-cert\/([0-9a-f]{32})$/;
const BASE64URL = /^[A-Za-z0-9_-]+$/;
const CALLBACK_PATH = '/api/v1/m365/comms-consent/callback';
const INTERNAL_AUTH_ISSUER = 'breeze-api' as const;
const INTERNAL_AUTH_AUDIENCE = 'm365-communications-executor' as const;
...
function parseCallbackUrl(source: Environment): string {
  const raw = required(source, 'M365_COMMS_CALLBACK_URL');
  let parsed: URL;
  try {
    parsed = new URL(raw);
  } catch {
    throw new Error('M365_COMMS_CALLBACK_URL must be the exact HTTPS consent callback URI');
  }
  if (
...
function parseVaultUrl(source: Environment): string {
  const raw = required(source, 'M365_COMMS_VAULT_URL');
  let parsed: URL;
  try {
    parsed = new URL(raw);
  } catch {
    throw new Error('M365_COMMS_VAULT_URL must be an HTTPS vault origin');
  }
  if (
    parsed.protocol !== 'https:'
    || !parsed.hostname
...
function parsePublicJwk(source: Environment, expectedKid: string): ExecutorInternalAuthPublicJwk {
  const raw = required(source, 'M365_COMMS_EXECUTOR_SIGNING_PUBLIC_JWK');
  let parsed: unknown;
  try {
    parsed = JSON.parse(raw);
  } catch {
    throw new Error('M365_COMMS_EXECUTOR_SIGNING_PUBLIC_JWK must contain valid public JWK JSON');
  }
  const result = publicJwkSchema.safeParse(parsed);
  if (!result.success || result.data.kid !== expectedKid) {
    throw new Error(
      'M365_COMMS_EXECUTOR_SIGNING_PUBLIC_JWK must be the configured Ed25519 public verification JWK and match M365_COMMS_EXECUTOR_SIGNING_KID',
    );
  }
  return result.data;
}
...
export function createAzureCredential(
  ...
  throw new Error(
    'M365_COMMS_EXECUTOR_AZURE_CREDENTIAL_MODE must be managed-identity or workload-identity',
  );
}
...
export function loadExecutorConfig(
  ...
  const clientCertVersion = required(source, 'M365_COMMS_CLIENT_CERT_VERSION');
  if (!CREDENTIAL_VERSION.test(clientCertVersion)) {
    throw new Error(
      'M365_COMMS_CLIENT_CERT_VERSION must be exactly 32 lowercase hex characters',
    );
  }

  const clientCertVaultRef = required(source, 'M365_COMMS_CLIENT_CERT_VAULT_REF');
  const vaultMatch = VAULT_REF.exec(clientCertVaultRef);
  if (
    !vaultMatch
    || vaultMatch[1] !== new URL(vaultUrl).host
    || vaultMatch[2] !== clientCertVersion
  ) {
    throw new Error(
      'M365_COMMS_CLIENT_CERT_VAULT_REF must match M365_COMMS_VAULT_URL and end with /m365-comms-client-cert/<M365_COMMS_CLIENT_CERT_VERSION>',
    );
  }
  ...
  const internalAuthKid = required(source, 'M365_COMMS_EXECUTOR_SIGNING_KID');
  const internalAuthPublicJwk = parsePublicJwk(source, internalAuthKid);

  const internalAuthIssuer = required(source, 'M365_COMMS_EXECUTOR_ISSUER');
  if (internalAuthIssuer !== INTERNAL_AUTH_ISSUER) {
    throw new Error(`M365_COMMS_EXECUTOR_ISSUER must equal ${INTERNAL_AUTH_ISSUER}`);
  }

  const internalAuthAudience = required(source, 'M365_COMMS_EXECUTOR_AUDIENCE');
  if (internalAuthAudience !== INTERNAL_AUTH_AUDIENCE) {
    throw new Error(`M365_COMMS_EXECUTOR_AUDIENCE must equal ${INTERNAL_AUTH_AUDIENCE}`);
  }
  ...
  const bindHost = required(source, 'M365_COMMS_EXECUTOR_BIND_HOST');
  if (!privateBindAddress(bindHost)) {
    throw new Error('M365_COMMS_EXECUTOR_BIND_HOST must be a private IP interface');
  }
  const rawPort = required(source, 'M365_COMMS_EXECUTOR_PORT');
  const port = Number(rawPort);
  if (!/^[1-9][0-9]{0,4}$/.test(rawPort) || !Number.isSafeInteger(port) || port > 65_535) {
    throw new Error('M365_COMMS_EXECUTOR_PORT must be an integer from 1 through 65535');
  }
  ...
  return {
    ...
    clientCertVersion,
    clientCertVaultRef,
    tokenCacheKekWriterVersion,
    tokenCacheKekVaultRef,
    tokenCacheKekReaderVersions,
    tokenCacheDsn,
    internalAuthKid,
