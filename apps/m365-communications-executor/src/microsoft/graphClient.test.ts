...
describe('graph post method (comms delta: patch deleted, post added)', () => {
  it('POSTs JSON to the fixed graph path and returns status with a null body on 202', async () => {
    ...
    expect(init).toMatchObject({
      ...
      body: JSON.stringify({ message: { subject: 's' } }),
    });
    expect((init as RequestInit).signal).toBeInstanceOf(AbortSignal);
  });

  it('returns the parsed JSON body on 201 (createMessage)', async () => {
    const fetchStub = vi.fn(async () => json({ id: 'draft-1', webLink: 'https://outlook.example/x' }, { status: 201 }));
    const graph = createMicrosoftGraphClient(
      { applicationId: APPLICATION_ID },
      { fetch: fetchStub as unknown as typeof fetch },
    );

    const result = await graph.post({ accessToken: ACCESS_TOKEN, path: '/me/messages', body: { subject: 's' } });

    expect(result).toEqual({ status: 201, body: { id: 'draft-1', webLink: 'https://outlook.example/x' } });
  });

  it('maps non-2xx statuses through readFailure like the read methods', async () => {
    await expect(clientWithStatus(403, { error: { code: 'ErrorAccessDenied' } })
      .post({ accessToken: ACCESS_TOKEN, path: '/me/sendMail', body: {} }))
      .rejects.toMatchObject({ code: 'graph_permission_missing' });
    await expect(clientWithStatus(404, {})
      .post({ accessToken: ACCESS_TOKEN, path: '/me/messages', body: {} }))
      .rejects.toMatchObject({ code: 'graph_not_found' });
    await expect(clientWithStatus(429, {}, { 'retry-after': '17' })
      .post({ accessToken: ACCESS_TOKEN, path: '/me/sendMail', body: {} }))
      .rejects.toMatchObject({ code: 'graph_throttled', retryAfterSeconds: 17 });
  ...
  it('times out with a sanitized code that never carries the access token', async () => {
    ...
    const failure = await graph.post({ accessToken: ACCESS_TOKEN, path: '/me/sendMail', body: {} })
      .catch((error: unknown) => error);
    expect(failure).toMatchObject({ code: 'graph_request_timeout', message: 'graph_request_timeout' });
    expect(JSON.stringify(failure)).not.toContain(ACCESS_TOKEN);
  });

  it('rejects a post path that does not start with a slash without a network call', async () => {
    const fetchStub = jsonResponder({});
    const graph = createMicrosoftGraphClient(
      { applicationId: APPLICATION_ID },
      { fetch: fetchStub as unknown as typeof fetch },
    );
    await expect(graph.post({ accessToken: ACCESS_TOKEN, path: 'me/sendMail', body: {} }))
      .rejects.toMatchObject({ code: 'graph_request_invalid' });
    expect(fetchStub).not.toHaveBeenCalled();
  });

  it('has no patch member — the comms executor mutates only via its two POST shapes', () => {
    const graph = createMicrosoftGraphClient(
      { applicationId: APPLICATION_ID },
      { fetch: jsonResponder({}) as unknown as typeof fetch },
    );
    expect((graph as unknown as Record<string, unknown>).patch).toBeUndefined();
  });
});

describe('readResource expand parameter (comms delta)', () => {
  it('joins $expand into the query exactly like $select', async () => {
    const fetchStub = jsonResponder({ id: 'm1' });
    const graph = createMicrosoftGraphClient(
      { applicationId: APPLICATION_ID },
      { fetch: fetchStub as unknown as typeof fetch },
    );
    await graph.readResource({
      accessToken: ACCESS_TOKEN,
      path: '/me/messages/m1',
      select: ['id', 'subject'],
      expand: 'attachments($select=id,name)',
    });
    const url = new URL(fetchStub.mock.calls[0]![0] as string);
    expect(url.searchParams.get('$select')).toBe('id,subject');
    expect(url.searchParams.get('$expand')).toBe('attachments($select=id,name)');
  });

  it('omits $expand entirely when not provided', async () => {
    const fetchStub = jsonResponder({ id: 'm1' });
    const graph = createMicrosoftGraphClient(
      { applicationId: APPLICATION_ID },
      { fetch: fetchStub as unknown as typeof fetch },
    );
    await graph.readResource({ accessToken: ACCESS_TOKEN, path: '/me/messages/m1', select: ['id'] });
    const url = new URL(fetchStub.mock.calls[0]![0] as string);
