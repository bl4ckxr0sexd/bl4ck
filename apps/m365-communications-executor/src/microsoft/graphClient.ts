...
export function createMicrosoftGraphClient(
  ...
  return {
    ...
    async readResource(input) {
      ...
      const query: Record<string, string> = { '$select': input.select.join(',') };
      if (input.expand !== undefined) query.$expand = input.expand;
      const body = await readRequest(
        graphUrl(input.path, query),
        input.accessToken,
        budget,
      );
    ...
    async post(input) {
      ...
      try {
        ...
        // sendMail answers 202 with no body; createMessage answers 201 with
        // JSON. Consume/bound the body either way; only translate non-2xx
        // into a typed failure.
        const responseBody = await readBoundedBody(response, budget, maxResponseBytes);
        if (!response.ok) throw readFailure(response, responseBody);
        return {
          status: response.status,
          body: responseBody.trim() === '' ? null : parseJson(responseBody),
        };
      } catch (error) {
        if (error instanceof GraphClientError) throw error;
        if (timedOut) throw failure('graph_request_timeout');
