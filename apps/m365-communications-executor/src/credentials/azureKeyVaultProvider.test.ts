...
const SECRET_NAME = 'm365-comms-client-cert';
...
describe('AzureKeyVaultCertificateProvider', () => {
  ...
  it.each([
    ...
    ['a different profile', `akv://customer-vault.vault.azure.net/m365-comms-other-secret/${SECRET_VERSION}`],
  ...
  it.each([
    ...
    ['an extra envelope field', JSON.stringify({ schemaVersion: 1, domain: 'communications-delegated', material: { kind: 'certificate', certificatePem: 'CERT', privateKeyPem: 'KEY' }, extra: true })],
    ['the wrong schema version', JSON.stringify({ schemaVersion: 2, domain: 'communications-delegated', material: { kind: 'certificate', certificatePem: 'CERT', privateKeyPem: 'KEY' } })],
    ['the wrong domain', JSON.stringify({ schemaVersion: 1, domain: 'customer-graph-actions', material: { kind: 'certificate', certificatePem: 'CERT', privateKeyPem: 'KEY' } })],
    ['a stored thumbprint', JSON.stringify({ schemaVersion: 1, domain: 'communications-delegated', material: { kind: 'certificate', certificatePem: 'CERT', privateKeyPem: 'KEY', thumbprint: 'THUMB' } })],
    ['an empty certificate', JSON.stringify({ schemaVersion: 1, domain: 'communications-delegated', material: { kind: 'certificate', certificatePem: '', privateKeyPem: 'KEY' } })],
    ['an extra material field', JSON.stringify({ schemaVersion: 1, domain: 'communications-delegated', material: { kind: 'certificate', certificatePem: 'CERT', privateKeyPem: 'KEY', refreshToken: 'SECRET' } })],
