...
export function WindowsIcon({ title = 'Windows', className, ...rest }: OSIconProps) {
  ...
  const iconTitle = title === 'Windows' ? t('osIcons.windows') : title;
  return (
    <svg
...
export function MacOSIcon({ title = 'macOS', className, ...rest }: OSIconProps) {
  const { t } = useTranslation('devices');
  const iconTitle = title === 'macOS' ? t('osIcons.macos') : title;
  return <Apple className={className} aria-label={iconTitle}>
    <title>{iconTitle}</title>
  </Apple>;
}
...
export function LinuxIcon({ title = 'Linux', className, ...rest }: OSIconProps) {
  const { t } = useTranslation('devices');
  const iconTitle = title === 'Linux' ? t('osIcons.linux') : title;
  return (
    <svg
      xmlns="http://www.w3.org/2000/svg"
      viewBox="0 0 24 24"
