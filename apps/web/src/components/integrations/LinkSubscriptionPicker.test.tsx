...
describe("LinkSubscriptionPicker", () => {
  it("explains that observation tracking detects drift without overwriting billing", async () => {
    ...
    expect(
      screen.getByText(
        "Track Pax8 reported quantity for drift (never overwrite BL4CK billing)",
      ),
    ).toBeInTheDocument();
  ...
  it("creates a new manual line then links to it", async () => {
    ...
    expect(screen.getByText("Enter a BL4CK billing quantity.")).toBeInTheDocument();
