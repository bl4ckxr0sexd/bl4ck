...
describe("M365CustomerGraphReadCard", () => {
  ...
  it.each([
    ...
    expect(screen.getByRole("button", { name: "Disconnect from BL4CK" })).toBeEnabled();
    if (canRetest) {
  ...
  it("warns that Microsoft consent remains, disconnects through runAction, and reloads", async () => {
    ...
    render(<M365CustomerGraphReadCard />);
    fireEvent.click(await screen.findByRole("button", { name: "Disconnect from BL4CK" }));

    await waitFor(() => expect(fetchWithAuthMock).toHaveBeenCalledTimes(3));
    expect(confirm).toHaveBeenCalledWith(
      "Disconnecting stops BL4CK from using this connection. Microsoft tenant-wide consent remains until a customer administrator removes it in Microsoft Entra.",
    );
    expect(runActionMock).toHaveBeenCalledTimes(1);
    expect(fetchWithAuthMock).toHaveBeenNthCalledWith(
      2,
  ...
  it.each([
    ["consent", "Connect"],
    ["retest", "Retest"],
    ["disconnect", "Disconnect from BL4CK"],
  ...
  it.each([
    ...
    ["disconnect", "Disconnect from BL4CK", "complete"],
    ["disconnect", "Disconnect from BL4CK", "reject"],
  ...
  it("uses at least 44px touch targets for every action", async () => {
    ...
    for (const name of ["Re-consent", "Retest", "Disconnect from BL4CK"]) {
  ...
  it("disables every mutation without organizations:write", async () => {
    ...
    expect(screen.getByRole("button", { name: "Disconnect from BL4CK" })).toBeDisabled();
  });
