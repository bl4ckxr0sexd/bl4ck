...
import { reconcileCustomerGraphActions } from './reconcile';
...
const expected: CanonicalAppRoleAssignment[] = [
  ...getM365PermissionProfile('customer-graph-actions').applicationPermissionAssignments!,
];

function reconcile(observedGrants: readonly CanonicalAppRoleAssignment[] | null) {
  return reconcileCustomerGraphActions({ ...PROOF, observedGrants }, { currentDate: VERIFIED_AT });
}

describe('reconcileCustomerGraphActions', () => {
  it('has exactly the two customer-graph-actions grants in its fixed manifest', () => {
    expect(expected).toHaveLength(2);
  });

  it('returns observed=2/missing=0/unexpected=0 for exactly the two actions grants', () => {
    const result = reconcile(expected);

    expect(result.outcome).toBe('active');
    expect(result.grantReconciliation).toBe('complete');
    expect(result.observedGrants).toHaveLength(2);
  ...
  it('lands a third role in unexpected when only the two expected grants are required', () => {
  ...
  it('returns the other grant as missing when only one of the two is observed', () => {
    const missing = expected[1]!;
  ...
  it('returns both missing and unexpected sets when both drift directions exist', () => {
    ...
    const result = reconcile([expected[0]!, unknownResource]);

    expect(result.outcome).toBe('missing_and_unexpected');
    expect(result.missingGrants).toEqual([expected[1]]);
  ...
  it('compares resource application and role IDs rather than display metadata', () => {
    ...
    const result = reconcile([expected[1]!, sameDisplayWrongRole]);
