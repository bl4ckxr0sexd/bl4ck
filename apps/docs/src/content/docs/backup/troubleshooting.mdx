...
2. Confirm the BL4CK agent is running on the device.
3. Check that Redis is operational — backup jobs are dispatched through the job queue.

If a manual backup request cannot be queued at all, BL4CK now marks the job **Failed** immediately with the dispatch error instead of leaving it queued forever.
...
**Fix:**
1. Confirm the destination device shows as **Online** in BL4CK.
2. Verify the BL4CK agent service is running and can reach the API.
...
BL4CK does not create simulated verification records or silent pending restores in this case. The action fails explicitly so operators can correct the underlying issue first.
