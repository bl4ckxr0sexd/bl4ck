...
If the device is offline, BL4CK returns an error immediately and does not create a backup job. If queue dispatch fails after job creation, the job is marked **Failed** with the dispatch error so the history stays accurate.
...
<Aside type="note">
  Restore jobs also use the same durable command pipeline. If a restore command times out, BL4CK marks the device command failed and propagates the timeout into the restore job result so it does not remain stuck in **Pending** or **Running** forever.
</Aside>
