...
<Aside type="note">
  Verification is a live agent operation. The target device must be online and connected to BL4CK before an integrity check or test restore can start.
</Aside>
...
If the device is offline, BL4CK shows `Device is offline. Verification requires a connected agent.` and the verification buttons stay disabled. If command dispatch fails for another reason, BL4CK shows the dispatch error and lets you retry after the underlying issue is fixed.
...
BL4CK runs verifications automatically on a schedule:
...
No setup is needed — automated verifications run as soon as backup is configured. If a device is offline when a scheduled verification window arrives, BL4CK skips that run and logs the skip instead of creating a synthetic verification record.
...
If the overview shows **No verification history**, that means BL4CK has no completed live verification for the device yet. Run a real verification once the device is online to populate readiness and recent-failure history.
