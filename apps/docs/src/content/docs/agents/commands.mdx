...
The BL4CK agent supports **120+ commands** organized into 30+ categories. Commands are sent from the API to agents over WebSocket, each with an `action` string and an optional JSON `payload`.
...
| Param | Type | Default | Description |
|---|---|---|---|
| `title` | string | `"BL4CK Agent"` | Notification title |
...
| `tooltip` | string | `"BL4CK Agent"` | Tooltip text |
| `menuItems` | array | -- | Array of `{ id, label, enabled }` menu items |

---

## BL4CK Agent Service Management
