...
Policy evaluation depends on device telemetry that the BL4CK agent collects and reports. The evaluation service queries the following database tables to build the evaluation context for each device:
