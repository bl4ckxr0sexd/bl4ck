...
Webhooks let you push real-time event notifications from BL4CK to any external HTTP endpoint. When a device enrolls, an alert fires, a script completes, or any other tracked event occurs, BL4CK sends a signed JSON payload to each webhook configured for that event type. Webhooks are scoped to an organization and support custom headers, HMAC-SHA256 signature verification, automatic retries with exponential backoff, and a full delivery history with per-delivery status tracking.
...
Webhooks subscribe to events emitted by the BL4CK event bus. Pass one or more event type strings in the `events` array when creating or updating a webhook.
...
When an event is emitted on the BL4CK event bus, the webhook delivery system routes it to all active webhooks in the same organization that are subscribed to that event type. Each delivery follows this sequence:
...
When a webhook has a signing secret, every delivery includes an HMAC-SHA256 signature in the `X-Breeze-Signature` header. Use this to verify that the payload was sent by BL4CK and has not been tampered with.
...
BL4CK validates webhook URLs at both creation time and delivery time to prevent SSRF (Server-Side Request Forgery) attacks.
