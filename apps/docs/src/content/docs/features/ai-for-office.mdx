...
2. Enter the customer's Entra tenant ID (BL4CK suggests it automatically if you already have a Microsoft 365 connection for that customer).
...
<Steps>
1. The end-user opens the BL4CK add-in's task pane in Excel, Word, PowerPoint, or Outlook.
2. The add-in uses their existing **Microsoft identity** -- no separate BL4CK login.
3. BL4CK verifies the identity, maps the Microsoft tenant to the correct organization, and checks that the partner is entitled, the org policy is enabled, and the user is allowed.
