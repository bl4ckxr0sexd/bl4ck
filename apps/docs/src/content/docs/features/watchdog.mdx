...
description: A companion service that monitors the BL4CK agent process, performs escalating recovery, and provides failover connectivity when the agent is down.
---

import { Steps, Aside } from '@astrojs/starlight/components';

The **BL4CK Watchdog** is a lightweight companion service that runs alongside the BL4CK agent on every managed device. It continuously monitors the agent process, automatically recovers it when it becomes unhealthy, and maintains a fallback connection to the BL4CK server when the agent is completely down.
...
| **Failover** | Recovery attempts exhausted; watchdog is communicating directly with the BL4CK server on behalf of the device |
...
When recovery is exhausted, the watchdog enters **Failover** mode and takes over communication with the BL4CK server. In this state the watchdog:
...
The watchdog reports its own health to the server through its own connection. That means the watchdog can be perfectly healthy while the main agent has gone quiet — a "silent agent" condition that earlier versions of BL4CK couldn't distinguish from a fully-offline device.
...
The watchdog also ships journal entries to the BL4CK server as diagnostic logs (component prefix `watchdog.*`), which you can query via the API — see [Watchdog Logs](#watchdog-logs-api) below.
