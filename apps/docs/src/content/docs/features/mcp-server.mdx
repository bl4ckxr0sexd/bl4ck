---
title: MCP Server
description: Connect Claude Desktop, Cursor, and other MCP-compatible AI clients to your BL4CK fleet via the Model Context Protocol.
---

import { Steps, Aside, Tabs, TabItem } from '@astrojs/starlight/components';

BL4CK exposes a built-in **Model Context Protocol (MCP)** server that lets external AI clients -- Claude Desktop, Cursor, and any other MCP-compatible application -- interact directly with your fleet. Instead of copying data between dashboards and chat windows, you can ask an AI assistant to query devices, triage alerts, run scripts, and manage patches, all within the client you already use.

The MCP server reuses the same tools, guardrails, and audit infrastructure as the built-in BL4CK AI assistant, so every operation is scoped to your organization and governed by the same tiered permission model.

---

## How MCP works

The [Model Context Protocol](https://modelcontextprotocol.io/) is an open standard that defines how AI applications discover and invoke tools and resources hosted by external servers. A single MCP connection gives the AI client a live catalogue of everything it can do inside BL4CK -- no custom plugin code required.

BL4CK implements the **2024-11-05** protocol version with three capabilities:
...
The BL4CK MCP server uses the **SSE + HTTP POST** transport, which is the standard remote MCP transport for server-deployed instances.
...
1. Navigate to **Settings > API Keys** in the BL4CK dashboard.
...
For AI clients that implement OAuth 2.1 with Dynamic Client Registration (DCR) and PKCE — including **Claude.ai** and **ChatGPT** — BL4CK can authorize MCP connections through a real OAuth handshake instead of pasted API keys. The user pastes the tenant's `/mcp/sse` URL into the client, signs in to BL4CK, grants the AI client a scoped session, and the assistant receives a refreshable access token bound to that user and organization. No API key is created or copied.

OAuth is enabled with the `MCP_OAUTH_ENABLED=true` server setting. Once enabled, BL4CK acts as a standards-compliant authorization server and advertises the following endpoints (all discoverable automatically by the client — there is nothing to configure by hand):
...
2. Add the BL4CK MCP server to the `mcpServers` section:

3. Restart Claude Desktop. You should see the BL4CK tools appear in the tool picker.
...
<Aside>
  Replace `your-breeze-instance.example.com` with your actual BL4CK API hostname and `brz_your_api_key_here` with the API key you created.
</Aside>
...
4. Save and reload the window. BL4CK tools will appear in Cursor's agent mode.
...
Tools are the primary way AI clients interact with BL4CK. Each tool maps to one or more fleet operations and is classified into a tier that determines how it executes. The MCP server filters the tool list based on the scopes attached to your API key -- you only see tools you are authorized to use.
...
The tools visible and callable through MCP are determined by the scopes on your API key. BL4CK uses a four-scope model for AI operations:
...
In addition to API key scopes, every tool call passes through the BL4CK RBAC system. The user who created the API key must have the appropriate role-based permissions for the resource and action. For example, calling `run_script` requires the `scripts.execute` permission on the creator's role.

---

## Rate limiting

Rate limiting is enforced at multiple levels to protect your fleet and the BL4CK API.
...
Beyond scope and RBAC checks, a sensitive tool call can require an explicit **out-of-band approval** before it executes. When step-up is required, BL4CK creates an approval request, pushes it to the user's trusted mobile device, and blocks the action until the user decides on their phone:
...
Every JSON-RPC request is recorded in the BL4CK audit log with:
...
// Response
{
  ...
  "result": {
    ...
    "instructions": "You are connected to BL4CK RMM — a multi-tenant RMM platform for MSPs. ..."
  }
}
```
