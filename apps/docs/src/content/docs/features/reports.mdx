...
<Aside type="note">
  Today this report is created through the report API -- `POST /reports` with `"type": "security_compliance_posture"` and a `config` object -- rather than from the report builder. Once saved, it generates and downloads exactly like any other report (it appears in your reports list as "Security & Compliance Posture"). Posture-specific `config` options include `sites` (limit to specific sites), `windowDays` (recency window for "current" patch/AV data), `minPasswordLength`, `maxLocalAdmins`, `maxAvDefinitionsAgeDays`, and `includeCis` (add the CIS pass-rate section).
</Aside>
