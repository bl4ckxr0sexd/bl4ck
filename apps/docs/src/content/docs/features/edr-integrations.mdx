...
BL4CK integrates with leading Endpoint Detection and Response (EDR) platforms to give you a unified view of endpoint security across your managed fleet. Instead of switching between your RMM and EDR consoles, you can view threat data, agent coverage, and incident details directly in BL4CK.
...
Both integrations are configured once at the **partner** level: with your scope selector set to **All orgs** you provide the vendor's API credentials, map each vendor's organizations/sites to your BL4CK organizations, and BL4CK syncs agent and threat data on a schedule. The synced data is attributed to the correct customer and matched to your existing BL4CK devices.

### Auto-Linking: How EDR Agents Map to BL4CK Devices

During each sync, BL4CK automatically attempts to match imported EDR agents to enrolled BL4CK devices. The matching logic differs slightly by vendor:

**Huntress**: Agents are matched by **hostname** (case-insensitive, trimmed). BL4CK compares the Huntress agent's hostname against both the `hostname` and `displayName` of every enrolled device in the organization. If a match is found, the Huntress agent and any associated incidents are linked to that BL4CK device.

**SentinelOne**: Agents are matched first by **hostname** (the `computerName` field), then by **IP address** as a fallback. If the SentinelOne agent's computer name matches a BL4CK device hostname (case-insensitive), the match is made. If no hostname match exists, BL4CK checks the agent's network interfaces against known device IP addresses.

Agents that cannot be matched appear as "unmapped" in the integration status dashboard. To resolve unmapped agents, either enroll the missing device in BL4CK or verify that the hostname in the EDR system matches the hostname reported by the BL4CK agent.
...
The Huntress integration syncs agent inventory and incident reports from your Huntress account into BL4CK. It supports both pull-based syncing (via the Huntress API) and push-based updates (via webhooks). Huntress agents are automatically matched to BL4CK devices, and incidents are linked to the affected device when a mapping exists.
...
3. Enter your **Huntress API Key** and **API Secret** as shown in the Huntress portal. Paste each value into its own field — do not paste a Base64-encoded combination of the two; BL4CK formats the API request automatically. Both values are encrypted at rest using AES before being stored.
4. Enter your **Deployment Account Key**. This key is found on the Huntress agent-download page in your Huntress portal and is required to enable one-click Huntress deployment from BL4CK's **Software → Deployment** catalog.
5. Optionally provide your **Huntress account ID**. (A custom API base URL can be set via the BL4CK API; the default is `https://api.huntress.io/v1`.)
...
<Aside>
  The Huntress integration is configured at the **partner** level — one Huntress account serves all of your customer organizations. Configure it with your scope selector set to **All orgs**; the connection and credential fields are only available in that view. You then map each Huntress organization to a BL4CK organization (see [Organization Mappings](#organization-mappings) below). Organization users can view the agents and incidents for their mapped org but cannot change the connection or mappings.
</Aside>
...
Huntress can push event notifications to BL4CK in real time via webhooks.

#### Getting the webhook URL and secret from BL4CK
...
<Steps>
1. The exact, region-correct **Webhook URL** to paste into Huntress, with a **Copy** button. The URL includes an `integrationId` query parameter that tells BL4CK which partner the events belong to — keep it in the URL.
2. A **Webhook Secret** field with a **Generate** button. Click **Generate** to create a strong secret, copy it (it is shown only once), paste the same value into Huntress's webhook configuration, then **Save** to store it. BL4CK keeps it encrypted and never displays it again.
</Steps>
...
1. BL4CK reads the **signature** from the `x-huntress-signature` header (fallback: `x-signature`).
2. BL4CK reads the **timestamp** from the `x-huntress-timestamp` header (fallback: `x-timestamp`).
3. BL4CK computes `HMAC-SHA256(secret, "{timestamp}.{body}")` and compares the result to the provided signature using a timing-safe comparison.
...
When BL4CK receives a Huntress webhook, it needs to determine which integration the event belongs to. The integration is resolved using (in order of priority):
...
Because the Huntress integration is partner-scoped, BL4CK discovers the organizations in your Huntress account during each sync and lets you map each one to a BL4CK organization. Mapping an organization attributes its agents and incidents to the correct customer, so organization users only see their own data.
...
3. For each Huntress organization, select the BL4CK organization it should map to.
4. The mapping saves immediately. Agents and incidents from that Huntress organization are attributed to the selected BL4CK organization.
</Steps>

To remove a mapping, clear the selected BL4CK organization. Unmapped Huntress organizations remain visible in the partner view, but their agents and incidents are no longer attributed to a specific customer.
...
| `deviceId` | UUID | Mapped BL4CK device (nullable if unmapped) |
...
| `deviceId` | UUID | Affected BL4CK device (nullable if unmapped) |
...
| `mappedAgents` | Agents matched to BL4CK devices |
| `unmappedAgents` | Agents without a BL4CK device match |
...
| `deviceId` | UUID | Filter by affected BL4CK device |
...
The SentinelOne integration provides deeper operational capabilities beyond data sync. In addition to importing agent and threat data, you can isolate devices and take threat actions (kill, quarantine, rollback) directly from the BL4CK dashboard -- commands are dispatched to the SentinelOne management console via API.
...
SentinelOne organizes agents into "Sites." BL4CK lets you map SentinelOne sites to BL4CK organizations, enabling multi-tenant EDR management from a single SentinelOne console.
...
3. For each SentinelOne site, select the BL4CK organization it should map to. Unmapped sites stay quarantined — their agents and threats are not attributed to any customer — until assigned.
4. The mapping saves immediately.
</Steps>

To remove a mapping, clear the selected BL4CK organization.

### Device Isolation

You can isolate compromised devices directly from BL4CK. The isolation command is sent to SentinelOne, which enforces network isolation at the endpoint level.

<Steps>
1. Select one or more devices in the BL4CK dashboard.
2. Click **Isolate** (or **Unisolate** to restore connectivity).
3. BL4CK identifies the corresponding SentinelOne agents and dispatches the isolation command.
...
| `mappedDevices` | Agents matched to BL4CK devices |
...
From the `/security/edr` fleet page or from a device's **Security** tab, you can escalate a Huntress incident or a SentinelOne threat directly into a tracked BL4CK incident. Click **Promote to incident** on the item, confirm the pre-filled title and severity (mapped from the EDR severity level), and BL4CK creates the incident and navigates you to it. The promotion records the source (Huntress or SentinelOne), the original title, any affected device, and the detection time as the incident's detected-at timestamp.
...
Once a Huntress or SentinelOne integration is connected, both vendors appear as built-in deployment packages under **Software → Deployment**. You can push the EDR agent to any managed device — or group of devices — with a single click, without leaving BL4CK.

### Huntress

Huntress deployment is fully built in: no binary upload is required. BL4CK uses the **Deployment Account Key** you entered during integration setup to generate and deliver the Huntress installer to the target device. Simply navigate to **Software → Deployment**, select **Huntress** from the catalog, choose your target devices, and click **Deploy**.
...
2. Navigate to **Software → Deployment** in BL4CK and open the **SentinelOne** catalog entry.
...
Trigger a sync from the BL4CK dashboard or API. The sync job is queued via the background queue and processes in the background.
...
| `POST` | `/organizations/map` | Yes (partner) | Map or unmap a SentinelOne site to a BL4CK organization (`{ integrationId, s1SiteId, orgId }`) |
...
| `s1.device_isolated` | A device isolation initiated from BL4CK completes in SentinelOne |
| `s1.threat_action_completed` | A threat action initiated from BL4CK (kill, quarantine, rollback) completes |
...
You must configure a webhook secret on the integration before BL4CK will accept webhook payloads. Update the integration with a `webhookSecret` value that matches what you configured in your Huntress dashboard.

### Huntress webhook returns 409 "Multiple active integrations match"

When multiple Huntress integrations share the same account ID, BL4CK cannot determine which one to route the webhook to. Include the `integrationId` as a query parameter on your webhook URL or in the `x-huntress-integration-id` header.
...
After syncing, some EDR agents may not be matched to BL4CK devices. This happens when the hostname or identifier in the EDR system does not correspond to any enrolled BL4CK device. Enroll the missing devices in BL4CK or verify the hostname matches.
