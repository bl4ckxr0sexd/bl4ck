---
title: Mobile App
description: Monitor and manage your fleet on the go with the BL4CK mobile app for iOS and Android.
---

import { Steps, Aside, Tabs, TabItem } from '@astrojs/starlight/components';

The BL4CK mobile app gives technicians and administrators on-the-go access to device monitoring, alert management, and remote actions from their iOS or Android device. Built with React Native and Expo, the app connects to the same BL4CK API used by the web dashboard through a dedicated set of mobile-optimized endpoints mounted at `/api/v1/mobile`. Push notifications keep you informed of critical alerts in real time, and biometric authentication provides a secure, convenient login experience.
...
1. A client — an AI assistant, MCP client, the BL4CK Helper, or the BL4CK agent intercepting a Windows UAC prompt — initiates an action that requires approval. BL4CK creates a pending approval request and pushes a notification to the user's trusted devices. (For a UAC elevation, the request shows the agent as the requesting client and the device hostname as the requesting machine.)
...
The mobile app authenticates through the core BL4CK auth endpoints:
...
All mobile endpoints enforce the BL4CK multi-tenant hierarchy:
...
| `app_version` | `varchar(50)` | BL4CK app version |
