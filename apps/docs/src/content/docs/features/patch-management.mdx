...
<Image src={patchesScreenshot} alt="BL4CK Patch Management showing update rings, compliance, and deployment controls" />
...
| **`custom`** | Internally published updates distributed through BL4CK's custom patch channel |
...
Each catalog entry pairs a winget package ID with a vendor, friendly name, category, and a default severity. When a device reports an available third-party update, BL4CK matches it to the catalog so the patch shows the correct vendor and a sensible severity even before any vulnerability data is available. Updates from applications not in the catalog still appear in the patches list, but without this enrichment.
...
BL4CK can optionally run an AI-assisted smoke test on a third-party release before you roll it out widely. The test installs the release via winget on a dedicated Windows test VM and uses an AI model to judge whether the upgrade succeeded from the command output.
...
Scheduled patch jobs are reliable across restarts. If a job's scheduled run is ever lost before it reaches the queue — for example during a service restart — BL4CK automatically detects the stranded job after its scheduled time and re-runs it, so no scheduled patch cycle is silently skipped.
...
| `exclusiveWindowsUpdate` | boolean | **Manage Windows Update exclusively through BL4CK.** When enabled, BL4CK disables the device's native Windows Update automatic-install channel, so updates on that device only arrive through BL4CK's approval rings and schedule — no unexpected OS-initiated installs or reboots outside your policy. Turning it back off restores Windows' native update behavior. **Windows-only** — has no effect on macOS or Linux devices. Default: `false` |
