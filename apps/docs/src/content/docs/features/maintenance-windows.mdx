...
Matching devices are rebooted right away -- BL4CK doesn't wait for the window to close first.

### Platform behavior

<Tabs>
<TabItem label="Windows">
BL4CK issues a scheduled reboot with a 15-minute grace period. The signed-in user gets a "save your work" warning about 5 minutes before the reboot fires.
</TabItem>
<TabItem label="Linux">
BL4CK schedules the reboot 15 minutes out (equivalent to running `shutdown -r +15`), which also warns any logged-in users.
</TabItem>
<TabItem label="macOS">
Not supported. BL4CK cannot detect a pending-reboot state on macOS, so macOS devices are never rebooted by this feature -- reboot them manually or through a script instead.
</TabItem>
</Tabs>
