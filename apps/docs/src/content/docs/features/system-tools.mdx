...
System Tools provide a suite of remote administration capabilities that let technicians inspect and manage managed devices directly from the BL4CK dashboard. You access System Tools from a device's detail page by opening the **Remote Tools** panel, which organizes all tools into a tabbed interface.

Every operation is dispatched to the BL4CK agent running on the target device via the command queue. All mutating actions (killing processes, starting/stopping services, editing the registry, uploading files, running/enabling/disabling scheduled tasks) are recorded in the audit log.
...
- **Agent service detection.** If you restart the BL4CK agent service itself, the UI displays a "Agent Restarting" banner and automatically polls until the agent reconnects.
