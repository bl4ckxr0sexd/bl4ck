...
The Script Builder AI is an inline AI assistant embedded in the BL4CK script editor. It uses a conversational interface to help you write, modify, and test scripts through natural language. The assistant has access to your device fleet, existing script library, alert data, and script execution history -- giving it the context it needs to generate scripts tailored to your environment.
...
Script Builder sessions are distinct from general BL4CK AI sessions. They are scoped to the `script_builder` type and have access to a curated set of tools specific to script authoring.
...
<Steps>
1. Open the **Script Editor** in the BL4CK dashboard.
