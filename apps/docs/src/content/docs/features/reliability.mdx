...
The BL4CK agent collects reliability telemetry from platform-specific event sources: Windows Event Log, macOS system logs, and Linux journal/syslog. Each heartbeat cycle, the agent sends a snapshot of recent events to the API, which stores the raw history and then triggers an asynchronous score computation via BullMQ (with an inline fallback if the queue is unavailable).
...
BL4CK therefore uses two weighting profiles based on device type:
...
<Aside type="note">
  ...
  v0.89.0 adds two further server-side corrections: memory/disk event IDs are only counted as hardware errors when they come from a genuine hardware source (see [Hardware Errors](#hardware-errors)), and failures of BL4CK's own services no longer count against the service-failure factor (see [Service Failures](#service-failures)). Both apply on the next recompute — affected scores rise without an agent update.
</Aside>
...
<Aside type="note">
  Failures of BL4CK's own agent, watchdog, and helper services do **not** count against the reliability score. These are most commonly logged during agent auto-updates, when the operating system briefly reports the BL4CK service as "terminated unexpectedly" — indistinguishable at the OS level from a real crash. The events are still recorded and remain visible in the failure counts and the offenders drill-down, annotated "from BL4CK services, not scored" — they just don't lower the score.
</Aside>
...
<Tabs>
  ...
  <TabItem label="Linux">
    ...
    **v0.86.0 classification improvements**: BL4CK now filters Linux reliability data server-side. Routine kernel chatter (USB and ACPI messages, and similar non-fault log noise) no longer inflates the hardware-error count, and routine per-application crash reports are weighted far below a kernel panic. Expect Linux scores to rise after the server is upgraded to v0.86.0 — this applies even to devices still on older agents, because the cleanup runs on the server.
  </TabItem>
</Tabs>
...
One action is available: **Ask AI about reliability** — hands the device's reliability snapshot to the BL4CK AI assistant for a plain-language diagnosis and suggested next steps.
...
The BL4CK AI assistant can query device reliability data through its built-in tool system. The `query_device_reliability` tool allows natural language questions about fleet reliability to be answered with real data.
