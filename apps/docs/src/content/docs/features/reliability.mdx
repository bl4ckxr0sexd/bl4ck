...
<Aside type="note">
  ...
  v0.89.0 adds two further server-side corrections: memory/disk event IDs are only counted as hardware errors when they come from a genuine hardware source (see [Hardware Errors](#hardware-errors)), and failures of Breeze's own services no longer count against the service-failure factor (see [Service Failures](#service-failures)). Both apply on the next recompute — affected scores rise without an agent update.
</Aside>
