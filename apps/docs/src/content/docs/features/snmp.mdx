...
2. **Poll dispatch** -- For each due device, the scheduler enqueues a `poll-device` job. The worker loads the device's template OIDs, finds an online BL4CK agent in the same organization, and sends an `snmp_poll` command over the agent's WebSocket connection.
