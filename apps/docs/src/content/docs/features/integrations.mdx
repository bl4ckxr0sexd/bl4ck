...
BL4CK supports three categories of integrations, each served by a dedicated set of API routes.
...
| **Network** | UniFi (Ubiquiti Site Manager cloud, or a self-hosted controller) | Discover UniFi fleet inventory and WAN health via the cloud, or connect a self-hosted controller directly through a BL4CK agent -- with optional agent-collected deep telemetry either way (see [UniFi Network](#unifi-network)) |
...
Communication integrations push notifications from BL4CK into your team's messaging platform. Each provider is configured with a POST request containing the webhook URL and any provider-specific settings.
...
Connect BL4CK to your accounting platform from the **Accounting** tab on the Integrations page. Accounting integrations are a partner-level feature -- they're set up once for your practice, not per customer.
...
2. You're taken to Intuit's consent screen to authorize BL4CK; approving it returns you to BL4CK with the connection established.
...
Once QuickBooks is connected, you can bring your existing QuickBooks customers into BL4CK as organizations -- no re-typing. Each imported customer becomes one BL4CK **organization** (customer type) with a default **site**, carrying over the billing contact and address.
...
3. Click **Import selected**. BL4CK reports how many were imported, skipped, and failed, with an inline list of any failures.
...
Distributor integrations live on the **Distributors** tab of the Integrations page and let you bring vendor licenses and pricing into BL4CK. They're partner-scoped.

### Pax8

Pax8 syncs your clients and their subscription/license quantities, which you can map to organizations and link to recurring contract lines. After connecting your Pax8 account and mapping companies to BL4CK organizations, open the **Subscriptions** tab to manage how each subscription connects to a contract line.
...
The UniFi integration discovers your Ubiquiti network fleet across every customer site and reconciles it into BL4CK's unified network view -- so gateways, switches, and access points show up alongside your agent-discovered assets without duplicating them. It's a **partner-level** integration set up once for your practice, and it's strictly read-only: BL4CK never changes your UniFi configuration.

There are two ways to connect it: through Ubiquiti's cloud (**Site Manager**), optionally paired with on-site deep telemetry agents for richer per-port and per-client detail, or fully **self-hosted** with no Ubiquiti cloud account at all, where a BL4CK agent talks directly to your on-prem controller and is the only path BL4CK uses to reach it.
...
Paste a single Ubiquiti **Site Manager API key** that covers your whole account, and BL4CK discovers every UniFi console and its sites. After you map each UniFi site to a BL4CK site, BL4CK syncs device inventory -- model, MAC, type, firmware version and whether an upgrade is available, uptime, and adoption state -- plus the latest WAN/ISP health per site.

<Steps>

1. Open **Integrations → UniFi**.
2. Generate a **Site Manager API key** in your Ubiquiti account (at unifi.ui.com / UniFi Site Manager).
3. Paste it into the **UniFi Site Manager API key** field and click **Connect to UniFi**. BL4CK validates the key before saving -- a bad key is rejected with a clear message, and a UniFi outage is reported separately so a working key isn't wrongly blamed.
4. In the **Site mapping** card, choose the matching **BL4CK site** (grouped by organization) for each discovered UniFi site, then **Save mappings**. The organization is derived from the site you pick; unmapped UniFi sites are simply not synced.
...
The cloud key is deliberately shallow: it can't see per-port PoE, throughput, or the clients attached to each device. To get that depth, designate a BL4CK **agent already running at the site** as a collector for a given console. That agent polls the console's local **Network Integration API** (read-only, requires controller firmware **9.3 or newer**) and reports back per-port PoE detail (power draw, link speed, mode), device health (uptime, CPU/memory, throughput, client count), and the currently-connected clients (hostname, IP, which AP or switch, wired vs Wi-Fi and SSID, signal strength).

<Steps>

1. In the **Deep telemetry collectors** card, for each console set the **BL4CK site** it serves, a **collector agent** (only online agents at that site can be selected), the **Controller URL** (for example `https://192.168.1.1`), and the console's **local Network Integration API key**.
2. Click **Enable deep telemetry**. BL4CK encrypts the local key, pushes the configuration to the chosen agent, and the agent begins polling on schedule (about once a minute).
...
<Aside>
  UniFi devices and connected clients flow into BL4CK's [Network Intelligence](/features/network-intelligence/) discovered-assets view, matched by MAC (then IP), enriching existing assets rather than creating duplicates.
</Aside>

### Self-hosted controllers (no cloud account)

If you'd rather not create a Ubiquiti cloud account at all, connect UniFi in **self-hosted mode** instead. In this mode there's no Site Manager API key anywhere -- a BL4CK agent that already has network access to your on-prem UniFi Network controller polls it directly over the same read-only local **Network Integration API** used for deep telemetry (controller firmware **9.3 or newer**). The agent is the primary, and only, path BL4CK uses to reach the controller.
...
2. In the **Controllers** card, register each controller: pick the **BL4CK site** it primarily serves, a **collector agent** (an existing enrolled agent at that site that can reach the controller over the LAN), the **Controller URL** (for example `https://192.168.1.1`), and the controller's **local API key**, then click **Register controller**. The local key is encrypted before storage and pushed to the chosen agent.
3. Once the assigned agent reaches the controller (within about a minute), the controller's own local sites are discovered automatically and appear in the **Site mapping** card. Map each discovered site to whichever **BL4CK site** it corresponds to, then save.
...
Because sites are mapped individually rather than the whole controller, **one self-hosted controller can feed many UniFi sites across several different BL4CK organizations** -- for example, an MSP running a single controller that manages several clients' networks only needs one collector agent and one controller registration; each of the controller's discovered local sites is then mapped to whichever organization it actually belongs to.

BL4CK polls the controller on an interval you can tune via the API (`pollIntervalSeconds`, roughly 15–3600 seconds); the default is about once a minute, matching the deep telemetry cadence.

<Aside type="note">
  In self-hosted mode the collector agent isn't an optional enhancement -- it's the only way BL4CK reaches your controller. If the assigned agent goes offline, polling for that controller stops until it reconnects or you point the registration at a different agent.
</Aside>
...
PSA connections can map BL4CK alerts and devices to external tickets. Ticket mappings track the relationship between internal entities and their corresponding tickets in the PSA platform.
...
| `raw.alertId` | Associated BL4CK alert ID (if any) |
| `raw.deviceId` | Associated BL4CK device ID (if any) |
...
| `alert_id` | UUID | Associated BL4CK alert (nullable) |
| `device_id` | UUID | Associated BL4CK device (nullable) |
...
Confirm that the credentials are correct and the PSA platform's API is accessible from your BL4CK API server. Some providers require IP allowlisting or VPN access. Check the `lastTestedAt` and sync status fields on the connection for additional context.
