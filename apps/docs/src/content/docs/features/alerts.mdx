---
title: Alerts & Rules
description: Create alert rules, manage notifications, and respond to device alerts in BL4CK RMM.
sidebar:
  label: "Alerts & Rules"
---
...
BL4CK has a built-in alert system that monitors your devices and notifies the right people when something goes wrong. Alerts are triggered by **rules** you define, routed through **notification channels** you configure, and managed from the **Alerts** page in the dashboard.
...
When multiple related alerts fire at once (for example, a network outage causing many devices to go offline), BL4CK automatically groups them using **alert correlation**. The correlation view shows:
...
Before deploying a rule to production, you can test it against a specific device. From the rule list, click the **Test** button, select a target device, and BL4CK will simulate condition evaluation and show you whether the rule would trigger.
...
For infrastructure-level alerting (monitoring the BL4CK platform itself with Prometheus and Alertmanager), see [Infrastructure Alerts](/monitoring/alerts/).
