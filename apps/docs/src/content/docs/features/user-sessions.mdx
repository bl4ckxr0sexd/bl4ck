...
1. **Agent starts the session collector**
   ...
   When the BL4CK agent starts, it initializes a `SessionCollector` that calls into the platform-specific `SessionDetector`. The detector uses native OS APIs to enumerate active sessions:
...
4. **Activity state mapping**
   ...
   Raw OS session states are normalized to the BL4CK activity state enum:

   | OS-Reported State | BL4CK State |
   |---|---|
...
Login and logout events are published to the BL4CK event bus in real time, enabling downstream automations and integrations:
