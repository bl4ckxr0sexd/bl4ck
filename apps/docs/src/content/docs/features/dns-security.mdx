...
Integrations connect BL4CK to your existing DNS filtering infrastructure. Each integration is scoped to an organization and stores encrypted API credentials for the target provider. Once connected, BL4CK periodically syncs DNS security events from the provider and stores them in the `dns_security_events` table, where they can be queried, filtered, and analyzed. Policies define domain-level allowlists and blocklists that are pushed back to the provider through a sync pipeline.
...
<Steps>
1. **Connect a DNS filter provider** by creating an integration with the provider's API credentials. BL4CK encrypts all secrets at rest.

2. **BL4CK syncs events** from the provider on a configurable interval. Each DNS query event is stored with its action (allowed, blocked, redirected), threat category, source device, and domain.
...
5. **Analyze threat data** using the stats, top-blocked, and events endpoints. BL4CK automatically aggregates data for efficient querying over long time ranges.
</Steps>
...
BL4CK supports the following DNS filter providers:
...
<Aside type="caution">
  **`config.piholeVersion` defaults to `v5` when not set.** If your Pi-hole instance is on v6, you must explicitly set `config.piholeVersion` to `v6` in the integration config — otherwise BL4CK will talk to it using the legacy v5 API, which will fail or behave unpredictably against a v6 instance.
</Aside>
...
<Aside>
  BL4CK also detects DNS filtering tools during management posture assessment. If Cisco Umbrella, DNSFilter, or Netskope agents are installed on a device, they appear in the device's posture report regardless of whether an API integration is configured.
</Aside>
...
When a blocked event arrives with a known threat category (malware, phishing, botnet, ransomware, cryptomining), BL4CK automatically raises a high-severity alert for the affected device. You don't need a rule for this — the alert fires from the DNS Security pipeline itself.
...
<Aside>
  For time ranges longer than 7 days, BL4CK automatically queries pre-computed aggregation tables (`dns_event_aggregations`) instead of scanning raw events. The `source` field in the response indicates whether results came from `raw` event data or `aggregated` summaries.
</Aside>
