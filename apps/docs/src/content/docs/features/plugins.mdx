---
title: Plugins
description: Extend BL4CK with installable plugins for integrations, automations, reporting, collectors, notifications, and UI customisation.
---

import { Steps, Aside, Tabs, TabItem } from '@astrojs/starlight/components';

Plugins extend BL4CK with additional capabilities without modifying the core platform. Each plugin is published to a central catalog, installed per-organization, and managed through a dedicated set of API endpoints mounted at `/plugins`.
...
| Type | Description |
|------|-------------|
| `integration` | Connects BL4CK to external services (PSA tools, CRMs, cloud providers) |
...
| `ui` | Extends the BL4CK web interface with custom views or widgets |
...
| `minAgentVersion` | Minimum BL4CK agent version required |
| `minApiVersion` | Minimum BL4CK API version required |
