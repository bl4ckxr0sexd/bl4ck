...
| `managed` | Linked to an enrolled BL4CK agent device |
...
Discovery builds an interactive map of how your network actually fits together -- a measured backbone of switches and routers with the hosts attached to them -- and lets you hand-map anything discovery can't see. Connections carry their provenance (how BL4CK knows about them) so you can tell measured links from ones you drew yourself. See [Network Topology](#network-topology) below.
...
From the **Monitoring** tab you can **Unlink** a device that was manually linked to an enrolled BL4CK device — the Unlink action only appears for manually-linked devices, since auto-linked devices are kept in sync automatically by future scans. To link an asset to a device for the first time, use the **Manage in Discovery** link near the top of the page, which opens the asset in the Discovery view where you can complete the link (see [Linking an Asset to a Device](#linking-an-asset-to-a-device) below) and reach Proxy Access (see [Connecting to a discovered device's web UI](#connecting-to-a-discovered-devices-web-ui)).

### Linking an Asset to a Device

Manually link a discovered asset to an enrolled device. This sets the asset status to `managed` and is used for **identity and asset tracking only** — it records which enrolled BL4CK device corresponds to this discovered asset. Linking alone does not open remote access; see [Connecting to a discovered device's web UI](#connecting-to-a-discovered-devices-web-ui) below for proxy access.
...
The asset panel on the **Discovery** page includes a **Proxy Access** section that lets you open a LAN device's built-in web interface (router admin page, NAS panel, printer UI, switch management port, etc.) directly inside BL4CK — no VPN required.
...
4. Click **Connect**. The device's web UI loads in an iframe inside BL4CK, with an **Open in new tab** option for a full-window view.

</Steps>

Traffic is tunneled over the bridge agent's existing WebSocket connection to BL4CK — no additional firewall rules or port forwarding required. See [Network Proxy](/features/remote-access/#network-proxy) in the Remote Access docs for allowlist configuration and the full security model.
...
The **Topology** tab on the Discovery page renders your network as an interactive map: subnets group their hosts, and lines show how infrastructure and endpoints connect. Every connection is labeled with how BL4CK learned it, so you can trust what you're looking at.

### How connections are measured

Instead of guessing relationships from IP subnets, BL4CK measures the backbone from the network devices themselves during SNMP-enabled scans:
