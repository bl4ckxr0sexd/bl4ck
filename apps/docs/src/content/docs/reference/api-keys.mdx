...
BL4CK RMM provides two distinct key types for different purposes:
...
API keys allow external systems to authenticate against the BL4CK API without a user session. Each key is tied to a single organization and can be scoped to restrict which operations it may perform.
...
Both key types enforce the BL4CK multi-tenant hierarchy:
