...
description: Configure OIDC and SAML identity providers for organization-level single sign-on in BL4CK RMM.
sidebar:
  order: 11
  label: SSO
---
...
| **Auto-provisioning** | When enabled (default), users who authenticate via SSO are automatically created in BL4CK if they do not already exist. |
...
| **Attribute mapping** | Maps IdP claim names to BL4CK user fields (`email`, `name`, `firstName`, `lastName`, `groups`). |
| **PKCE** | Proof Key for Code Exchange. BL4CK uses PKCE (S256 method) for all OIDC authorization requests to prevent authorization code interception. |
...
BL4CK ships with built-in presets that auto-configure scopes, attribute mappings, and discovery URLs:
...
<Steps>
1. **Register BL4CK in your IdP.** Create an application/client in your identity provider. Set the redirect URI to `https://breeze.yourdomain.com/api/v1/sso/callback`. Note the Client ID and Client Secret.

2. **Create the provider in BL4CK.** Use the `POST /api/v1/sso/providers` endpoint. If you specify a `preset` and an `issuer`, BL4CK will auto-discover the authorization, token, userinfo, and JWKS endpoints via the `.well-known/openid-configuration` document.

3. **Test the configuration.** Use the `POST /api/v1/sso/providers/:id/test` endpoint to verify that BL4CK can reach the IdP's discovery document.
...
When you provide an `issuer` URL and the provider type is `oidc`, BL4CK automatically fetches the IdP's `.well-known/openid-configuration` document and populates:
...
The BL4CK SSO service provides a full SAML 2.0 toolkit:
...
- **Attribute mapping** -- Maps SAML attribute URIs (e.g., `http://schemas.xmlsoap.org/ws/2005/05/identity/claims/emailaddress`) to BL4CK user fields.

### Service Provider Metadata

BL4CK generates SP metadata with the following values:
...
BL4CK automatically maps common SAML attribute URIs to user fields:

| SAML Attribute URI | BL4CK Field |
|--------------------|--------------|
...
7. **BL4CK validates the browser binding, then claims the state.** The `state` returned by the IdP is verified against the `breeze_sso_state` cookie using a constant-time comparison (`crypto.timingSafeEqual`) before anything else runs — a callback that did not originate from the same browser that started the flow is rejected. Only after the binding check does BL4CK atomically consume the session row (`DELETE ... RETURNING` on a matching, non-expired `state`), which makes each state strictly single-use and immune to replay.
8. **Code exchange.** BL4CK exchanges the authorization code for tokens at the IdP's token endpoint, including the PKCE `codeVerifier`.
9. **ID token verification.** If an ID token is returned, BL4CK verifies the issuer, audience, expiration, and nonce claims.
10. **User info retrieval.** BL4CK calls the IdP's UserInfo endpoint to get the user's profile.
11. **Attribute mapping.** The IdP's claims are mapped to BL4CK user fields using the configured `attributeMapping`.
12. **Domain check.** If `allowedDomains` is set, the user's email domain is validated.
13. **User provisioning.** If the user does not exist and `autoProvision` is enabled, a new BL4CK user is created and assigned to the organization with the provider's `defaultRoleId`.
14. **Session creation.** BL4CK creates a JWT token pair (access + refresh) and a session record.
...
```
Browser          BL4CK API          Identity Provider
  │                  │                      │
  │── GET /sso/login/:orgId ──►│            │
  │                  │── Generate PKCE ──►  │
  │◄── 302 Redirect ─│──────────────────►   │
...
| `attributeMapping` | `object` | No | From preset or `{ email: "email", name: "name" }` | Maps IdP claims to BL4CK fields. |
| `autoProvision` | `boolean` | No | `true` | Create BL4CK users on first SSO login. |
...
When a user authenticates via SSO, BL4CK follows this logic:

<Steps>
1. **Email lookup.** The mapped `email` attribute is used to find an existing BL4CK user (case-insensitive).
...
3. **No existing user + auto-provision disabled.** The login is rejected with a "user not found" error. An administrator must create the BL4CK account first.
4. **No existing user + auto-provision enabled.** A new BL4CK user is created with the mapped email and name. The user is added to the organization with the provider's `defaultRoleId`. SSO-provisioned users have no password set.
...
Users can link an SSO identity to their existing BL4CK account themselves, from **Settings → Profile → Connect SSO**. Linking runs the normal sign-in round trip at the IdP and then attaches the resulting identity to the signed-in account, so future SSO logins match by identity rather than email.

If linking fails, the profile page surfaces one of these errors:

| Error | Meaning |
|-------|---------|
| `email_mismatch` | The email asserted by the IdP does not match your BL4CK account's email. |
| `identity_in_use` | This IdP identity is already linked to a different BL4CK user. |
...
| `attribute_mapping` | `jsonb` | Maps IdP claims to BL4CK fields. |
...
|--------|------|-------------|
| `id` | `uuid` | Primary key. |
| `user_id` | `uuid` | BL4CK user (foreign key). |
...
|----------|----------|-------------|
| `PUBLIC_URL` or `PUBLIC_APP_URL` or `DASHBOARD_URL` | Yes (one of) | Base URL of the BL4CK dashboard. Used to construct the SSO callback URI (`{base}/api/v1/sso/callback`). Falls back to `http://localhost:3000`. |
...
BL4CK could not fetch the `.well-known/openid-configuration` document from the issuer URL. Check that:
- The issuer URL is correct and reachable from the BL4CK API server.
...
The user does not exist in BL4CK and `autoProvision` is disabled on the provider. Either:
...
The user exists in BL4CK but does not have an `organization_users` record for the provider's organization. Add the user to the organization or enable auto-provisioning with a `defaultRoleId`.
...
An unhandled error occurred during the callback. Check the BL4CK API server logs for the full error message. Common causes include:
- Network connectivity issues between BL4CK and the IdP's token endpoint.
- Mismatched `clientSecret` (the secret in the IdP does not match what was configured in BL4CK).
