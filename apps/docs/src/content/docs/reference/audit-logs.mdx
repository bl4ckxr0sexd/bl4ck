---
title: Audit Logs
description: Track every action across your BL4CK environment with comprehensive audit trails, agent diagnostics, and event logs.
sidebar:
  order: 6
  label: Audit Logs
---
...
BL4CK RMM provides three distinct logging systems that together give you full visibility into platform activity, agent health, and device-level events. Each system serves a different purpose and is stored in its own database table with dedicated API endpoints.
...
| **Agent Diagnostic Logs** | Capture internal agent runtime diagnostics (debug, info, warn, error) | BL4CK agent | `agent_logs` |
...
Platform audit logs record every significant action taken within your BL4CK environment. They are the primary compliance and security trail.
...
BL4CK tracks four types of actors:
...
| `agent` | A BL4CK agent running on a managed device | Submitting event logs, heartbeat updates |
...
Agent diagnostic logs capture internal runtime information from the BL4CK agent running on managed devices. These are useful for troubleshooting agent connectivity, update failures, and component-level issues.
...
For real-time inspection, BL4CK can query the Windows Event Log (or equivalent) directly on a device by sending commands through the agent. These are available under both the `/system-tools/` and `/devices/` route namespaces.
