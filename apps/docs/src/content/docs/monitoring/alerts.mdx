...
description: Configure Prometheus and Alertmanager rules to monitor the BL4CK platform itself.
sidebar:
  order: 1
...
These alerts monitor the BL4CK platform itself -- API health, database, Redis, and disk usage. For alerts about your managed devices, see [RMM Alerts](/features/alerts/).

If you deploy the optional observability stack (`docker-compose.monitoring.yml`), BL4CK supports infrastructure-level alerting through Prometheus and Alertmanager.

<Aside type="tip">
  In-app alerts monitor your managed devices. Infrastructure alerts (this page) monitor the BL4CK servers, database, and queue. Most organizations use both.
</Aside>
