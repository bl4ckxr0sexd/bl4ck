...
description: Skip the BL4CK login form when a deployment sits behind Cloudflare Access using the same identity provider.
sidebar:
  order: 9
...
When you put BL4CK behind Cloudflare Access (or any Zero Trust gateway that mints CF Access JWTs) and use the same identity provider that you've configured users for in BL4CK, the user ends up authenticating twice: once at the CF Access edge, once at the BL4CK login form. Enabling Cloudflare Access JWT trust lets a valid `Cf-Access-Jwt-Assertion` header short-circuit `POST /api/v1/auth/login` and mint a BL4CK session directly.
...
1. The browser hits the BL4CK SPA. Cloudflare Access authenticates the user against your IdP and attaches a signed `CF_Authorization` cookie plus the `Cf-Access-Jwt-Assertion` request header.
...
5. On any failure — flag off, header absent, invalid signature, expired token, wrong audience, JWKS unreachable, user not in BL4CK, user inactive — it falls through to the existing password handler. The browser then sees the normal login form.
...
| User from JWT email not present in BL4CK | Fall through | Password handler will 401 with the same generic error, no email enumeration |
...
- `CF_ACCESS_TRUSTS_MFA=false` (default) — even on a valid JWT, if the matched BL4CK user has MFA enrolled, the middleware issues a `tempToken` and the SPA's normal MFA challenge runs. The user does not have to re-enter their password, but they do enter their TOTP code.
- `CF_ACCESS_TRUSTS_MFA=true` — the minted BL4CK session is marked as MFA-satisfied. Only turn this on if your CF Access policy actually requires step-up. The setting is honoured deployment-wide; it does not vary per partner or per user.

## Redirect login flow

The header short-circuit on `POST /api/v1/auth/login` covers the normal SPA login XHR, but top-level browser navigations (deep links, an SSO-style redirect into BL4CK) can't attach a Bearer token or read a JSON response. For that case there is a dedicated `GET /api/v1/auth/cf-access-login` endpoint:
...
3. On success it mints a BL4CK token pair, sets the refresh cookie, and issues a `302` to the sanitized `next` path with a `?cf-access-login=success` marker. The SPA detects the marker and bootstraps from the refresh cookie.
...
| `CF_ACCESS_AUD` | Yes | The AUD tag for the Cloudflare Access application that protects BL4CK. Get it from the Cloudflare Zero Trust dashboard → Access → Applications → your app → AUD. |
...
- Identity provider: pick the same IdP whose `email` claim is the same one your BL4CK users are provisioned with. If a BL4CK user's email is `alice@acme.com` and your IdP issues the JWT with `email=alice@acme.com`, you're set.
- Session duration: anything you like. BL4CK mints its own refresh token independently of the CF Access cookie.
