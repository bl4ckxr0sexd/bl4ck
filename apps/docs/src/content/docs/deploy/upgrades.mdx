...
Org-level defaults override Partner-level defaults using the same hierarchical precedence that governs other policy settings — a per-org window takes priority over the partner-wide window.
