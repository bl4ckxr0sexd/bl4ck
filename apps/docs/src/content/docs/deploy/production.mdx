...
description: Deploy BL4CK RMM to production with Docker Compose and automatic TLS.
sidebar:
  order: 1
...
BL4CK ships as pre-built Docker images on GitHub Container Registry. A single `docker compose up -d` brings up a fully working production stack with automatic TLS.
...
3. **Set your domain and secrets**
   ...
   # Trust anchor (raw base64 Ed25519 public key) for the official BL4CK GitHub
   ...
   If BL4CK is behind a reverse proxy, also set `TRUST_PROXY_HEADERS=true` and list the proxy's CIDRs in `TRUSTED_PROXY_CIDRS`. See [Environment Variables](/deploy/environment/) for details.
