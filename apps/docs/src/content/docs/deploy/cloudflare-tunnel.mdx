...
description: Run BL4CK behind a Cloudflare Tunnel with no inbound ports, and optionally lock the dashboard behind Cloudflare Access.
sidebar:
  order: 5
...
A [Cloudflare Tunnel](https://developers.cloudflare.com/cloudflare-one/connections/connect-networks/) lets you expose BL4CK without opening any inbound ports on your server. The `cloudflared` daemon makes an outbound connection to Cloudflare's edge, and all traffic reaches BL4CK through that tunnel. Your origin IP stays hidden, and the firewall can deny all inbound traffic.
...
- A working BL4CK stack reachable internally on the Caddy container.
...
2. **Add a public hostname**
   ...
   In the tunnel's **Public Hostname** tab, add your BL4CK hostname (for example `breeze.example.com`) and point its **Service** at the Caddy container. The service type must be **HTTP**, not HTTPS — see [TLS Termination](#tls-termination-http-origin-required) above.
...
4. **Tell BL4CK it sits behind a proxy**
...
BL4CK has no built-in IP allowlist for the dashboard, so IP or identity restrictions belong at the proxy. With a tunnel already in place, [Cloudflare Access](https://developers.cloudflare.com/cloudflare-one/applications/) is the cleanest way to gate the admin UI while leaving agent traffic open to the internet.
