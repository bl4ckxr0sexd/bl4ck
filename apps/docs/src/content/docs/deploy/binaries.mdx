...
description: How BL4CK serves agent and viewer downloads — GitHub Releases, local disk, or S3.
sidebar:
  order: 5
...
BL4CK serves agent binaries and viewer installers through its own API. The download source is configurable via `BINARY_SOURCE`, so downloads work regardless of how you deploy.
...
For the official BL4CK releases, use the published trust anchor:
