...
The BL4CK `docker-compose.yml` includes a bundled coturn container that works when the server has a public IP. However, if your BL4CK stack runs behind Cloudflare Tunnel (or any other tunnel/proxy that only handles HTTP), **TURN must run on a separate host** because TURN uses UDP, which tunnels cannot carry.

## Architecture Options

| Setup | TURN Location | When to use |
|---|---|---|
| **Bundled** | Same host as BL4CK | Server has a public IP with UDP ports open |
| **External** | Dedicated VPS | BL4CK is behind Cloudflare Tunnel, NAT, or a load balancer that doesn't pass UDP |
...
## Configure BL4CK to Use External TURN

### 1. Update `.env`

Add or update these variables in your BL4CK `.env` file:
...
If your BL4CK server has a public IP (not behind a tunnel), the bundled coturn container works out of the box. Set these in `.env`:
...
Then start BL4CK with the `turn` profile enabled:
...
BL4CK uses the TURN **time-limited credential** mechanism (RFC 5766 long-term credentials with a shared secret):
...
- Ensure the firewall allows **both TCP and UDP** on port 3478 and the relay port range — BL4CK advertises `transport=tcp` and `transport=udp` TURN URLs
...
- Verify `static-auth-secret` matches `TURN_SECRET` in BL4CK's `.env`
